/**
 * Product Service — Products & Categories Management
 * Port: 3030
 */
...
app.use((req: Request, _res: Response, next: NextFunction) => {
  ...
  // Attach requestId to request object for downstream logging if needed
...
// Public routes (no Gateway/Auth required for read-only)
// Note: In a strict mesh, even public routes might require Gateway headers.
// We'll apply requireGateway to all routes for consistency as per plan.
app.use("/v1/products", requireGateway, productRoutes);
...
app.use((err: unknown, _req: Request, res: Response, _next: NextFunction) => {
  logger.error("Unhandled error in product service", err);
  res
    .status(500)
    .json(
